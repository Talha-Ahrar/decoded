...
export async function GET(request) {
  try {
    ...
    await ensureCollection(db, 'newscategories');
    
    const { searchParams } = new URL(request.url);
    const action = searchParams.get('action');

    if (action === 'list') {
      const categories = await db.collection('newscategories')
        .find({})
        .sort({ createdAt: -1 })
        .toArray();
  ...
  try {
    ...
    switch (action) {
      case 'create': {
        ...
        const existingCategory = await db.collection('newscategories').findOne({ name });
        if (existingCategory) {
          return NextResponse.json({ success: false, error: 'Category already exists' }, { status: 400 });
        }

        const category = await db.collection('newscategories').insertOne({
          name,
