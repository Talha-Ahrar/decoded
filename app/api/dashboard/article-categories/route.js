// app/api/dashboard/article-categories/route.js
...
export async function GET(request) {
  try {
    ...
    await ensureCollection(db, 'articlecategories');
  ...
  try {
    ...
    switch (action) {
      case 'create': {
        ...
        const existingCategory = await db.collection('articlecategories').findOne({ name });
        if (existingCategory) {
          return NextResponse.json({ success: false, error: 'Category already exists' }, { status: 400 });
        }

        const category = await db.collection('articlecategories').insertOne({
          name,
      ...
      case 'updateStatus': {
        ...
        const result = await db.collection('articlecategories').findOneAndUpdate(
