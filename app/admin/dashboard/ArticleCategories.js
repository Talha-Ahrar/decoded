...
export default function ArticleCategories() {
  ...
  const fetchCategories = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/dashboard/article-categories?action=list');
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch('/api/dashboard/article-categories', {
        method: 'POST',
  ...
  const handleUpdateStatus = async (categoryId, isActive) => {
    ...
    try {
      const res = await fetch('/api/dashboard/article-categories', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'updateStatus', categoryId, isActive: !isActive })
      });
      const data = await res.json();
      if (data.success) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <h1 className="text-2xl font-bold mb-6">Article Category Management</h1>
              ...
              <tr key={category._id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <button
                    ...
                    className="px-4 py-1 text-white rounded bg-indigo-500 hover:bg-indigo-600"
                  >
                    Toggle Status
                  </button>
                </td>
