...
export default function NewsCategoryManagement() {
  ...
  const fetchCategories = async () => {
    try {
      const response = await fetch('/api/dashboard/news-categories?action=list');
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch('/api/dashboard/news-categories', {
        method: 'POST',
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <h1 className="text-2xl font-bold mb-6">News Category Management</h1>
